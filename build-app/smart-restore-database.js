...
/**
 * Умное восстановление базы данных с проверкой существования таблиц
 */
async function smartRestoreDatabase() {
  try {
    console.log("🔄 Умное восстановление базы данных...");
    ...
    existingTables[0].forEach(table => {
      console.log(`- ${table.table_name}`);
    });
    
    // Очищаем только существующие таблицы в правильном порядке
    console.log("🗑️ Очистка существующих таблиц...");
    
    const tableNames = existingTables[0].map(t => t.table_name);
    
    // Очищаем в правильном порядке (сначала зависимые)
    const cleanupOrder = ['orders', 'orderItem', 'cart', 'transaction', 'review', 'advertisements', 'materials', 'materialcategories', 'user'];
    ...
    for (const tableName of cleanupOrder) {
      if (tableNames.includes(tableName)) {
        try {
          ...
          console.log(`❌ Ошибка при очистке ${tableName}: ${error.message}`);
        }
      }
    }
    
    console.log("✅ Существующие таблицы очищены");
    
    // Сбрасываем счетчики для существующих таблиц
    console.log("🔄 Сброс счетчиков...");
    
    const sequenceResets = [
      { table: 'user', sequence: 'user_id_seq' },
      { table: 'materialcategories', sequence: 'materialcategories_id_seq' },
      { table: 'materials', sequence: 'materials_id_seq' },
      { table: 'advertisements', sequence: 'advertisements_id_seq' },
      { table: 'orders', sequence: 'orders_id_seq' },
      { table: 'transactions', sequence: 'transactions_id_seq' },
      { table: 'carts', sequence: 'carts_id_seq' },
      { table: 'reviews', sequence: 'reviews_id_seq' }
    ];
    
    for (const { table, sequence } of sequenceResets) {
      if (tableNames.includes(table)) {
        try {
          await db.sequelize.query(`ALTER SEQUENCE "${sequence}" RESTART WITH 1`);
          console.log(`✅ Счетчик ${sequence} сброшен`);
        } catch (error) {
          console.log(`❌ Ошибка при сбросе счетчика ${sequence}: ${error.message}`);
        }
      }
    }
    ...
    // Создаем транзакции для заказов
    ...
    const transactions = await db.transaction.bulkCreate([
      {
        userId: users[0].id, // test
        orderId: orders[0].id,
        amount: -7000, // списание
        type: 'purchase',
        description: 'Покупка: Стеновые блоки',
        status: 'completed',
        createdAt: new Date(),
        updatedAt: new Date()
      },
      {
        userId: users[1].id, // test1
        orderId: orders[1].id,
        amount: -22500, // списание
        type: 'purchase',
        description: 'Покупка: Цемент М500',
        status: 'completed',
        createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
    ...
    console.log("  test2 / test2test2");
    console.log("  admin / adminadmin (админ)");
