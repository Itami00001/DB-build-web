...
/**
 * Принудительная очистка и создание через seed
 */
async function forceCleanSeed() {
  try {
    console.log("🔄 Принудительная очистка и seed...");
    
    // Подключаемся к базе данных
    await db.sequelize.authenticate();
    console.log("✅ Подключение к базе данных установлено");
    
    // Удаляем все таблицы через SQL
    ...
    // Создаем материалы через bulkCreate
    ...
    console.log(`🎉 Итог через Sequelize: ${finalCategories.length} категорий, ${finalMaterials.length} материалов`);
