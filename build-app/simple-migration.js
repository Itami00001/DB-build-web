...
/**
 * Простая миграция только для материалов и категорий
 */
async function simpleMigration() {
  try {
    console.log("🔄 Простая миграция материалов...");
    
    // Подключаемся к базе данных
    await db.sequelize.authenticate();
    console.log("✅ Подключение к базе данных установлено");
    
    // Удаляем только таблицы материалов и категорий
    ...
    // Создаем таблицы через существующие модели
    await db.materialCategories.sync({ force: false });
    await db.material.sync({ force: false });
    ...
    console.log("🎉 Простая миграция успешно завершена!");
    
    process.exit(0);
    
  } catch (error) {
    console.error("❌ Ошибка при простой миграции:", error);
    process.exit(1);
  }
}
