...
/**
 * Финальное исправление таблиц с правильными моделями
 */
async function finalFixTables() {
  try {
    console.log("🔄 Финальное исправление таблиц...");
    ...
    // Создаем таблицы через модели с правильными именами
    await db.materialcategories.sync({ force: true });
    await db.materials.sync({ force: true });
    ...
    const materials = await db.materials.bulkCreate([
    ...
    const finalMaterials = await db.materials.findAll({
      include: [
        {
    ...
    console.log("🎉 Финальное исправление таблиц завершено!");
